...
- Tolong ubah format angka pada kolom "Jumlah" di tabel ini. Gunakan format Rupiah standar Indonesia: tambahkan prefix "Rp " di depan, gunakan titik sebagai pemisah ribuan, dan hapus angka desimal di belakang koma (.00). Contoh hasil yang diharapkan: "Rp 600.000". Pastikan logic databasenya tidak ada yang berubah, cukup sesuaikan format tampilannya (UI) saja.
- Ubah tampilan section "Aksi Cepat" ini menjadi layout Grid. Buat setiap item di dalamnya (Pelanggan, Invoice, Bayar, Paket) menjadi bentuk "Card" atau "Button". Berikan background color yang tipis (misalnya bg-gray-50 atau bg-primary-50 di Tailwind), tambahkan border-radius (rounded-lg), dan padding yang proporsional. Berikan juga efek hover pada interaksinya agar user tahu bahwa elemen tersebut bisa diklik.
- Tolong hapus seluruh block kode section "Ringkasan" (yang menampilkan Total Paket, Pelanggan Aktif, Belum Bayar, Terisolir) beserta layout container-nya. Hapus hanya pada bagian UI ini saja karena datanya berulang dengan widget di bagian atas.
- Ubah konfigurasi warna untuk komponen badge di kolom "Metode" pada tabel ini agar lebih kontras. Terapkan color-coding berikut: jika valuenya 'cash' gunakan warna hijau (success), jika 'transfer' gunakan warna biru (info), dan jika 'qris' gunakan warna ungu atau pink. Terapkan menggunakan utility class Tailwind atau Filament traits yang sesuai.
- Modifikasi tampilan "empty state" pada komponen tabel ini. Ubah icon silang (X) menjadi icon centang hijau (check-circle) atau icon senyum/jempol. Kemudian, ubah teks "No invoices" menjadi kalimat yang lebih humanis dan positif, misalnya: "Mantap! Tidak ada pelanggan yang menunggak."
- Pindahkan informasi "SERVER LOAD: OPTIMAL" dan "PENDING: 0 Tagihan" dari header section sapaan. Ubah bentuk UI-nya menjadi badge atau pill kecil, lalu tempatkan posisinya sejajar di pojok kanan atas halaman (misalnya di area topbar navigasi atau di sebelah bar pencarian), sehingga area sapaan "Selamat Pagi" terlihat lebih bersih.
